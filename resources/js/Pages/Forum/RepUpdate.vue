    ...
    <app-layout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Updating Reply to: <b>{{ que.title }}</b>
            </h2>
        </template>
        ...
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="py-4 px-6">
                        <form method="post" @submit.prevent="updateAnswer">
                            <div>
                                <label class="block mb-2 font-medium text-sm text-gray-700" for="title">
                                    <span>Update your answer</span>
                                </label>
...
<script>
    ...
    export default {
        ...
        methods: {
            updateAnswer() {
                axios.put('/question/' + this.que.slug + '/reply/' + this.reply.id + '/edit', this.form)
                    .then(response => {
                        this.checkErrors = {
